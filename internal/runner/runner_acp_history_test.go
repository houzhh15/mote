...
// TestRunACPMode_HistoryCompression verifies that ACP mode compresses history when needed
func TestRunACPMode_HistoryCompression(t *testing.T) {
	...
	t.Logf("Provider received %d messages after potential compression", len(mockProv.receivedMessages))
	
	// The message count should be reasonable (not 101+ messages)
	// With compression, we expect significantly fewer messages
	if len(mockProv.receivedMessages) > 100 {
		t.Errorf("expected compressed message count (<100), got %d", len(mockProv.receivedMessages))
	}
}
